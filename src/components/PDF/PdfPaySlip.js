...
const PdfPaySlip = () => {
  ...
  const [mediaheader, setMediaHeader] = React.useState();
  ...
  console.log('sitemedia',selectedLocation)  
  ...
  const getHeader = () => {
    api
      .post('/site/getsiteMediaById', { site_id: selectedLocation })
      .then((res) => {
        setMediaHeader(res.data.data[0]);
      })
      .catch(() => {
        message('payroll Data Not Found', 'info');
      });
