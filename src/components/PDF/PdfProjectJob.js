...
const PdfProjectJob = ({id,JobId}) => {
  PdfProjectJob.propTypes = {
    id: PropTypes.any,
    JobId:PropTypes.any,
  }
  ...
  const getJobOrder = () => {
    api.post('/project/getTabQuoteById', { project_id: id }).then((res) => {
      ...
      console.log('Job', res.data.data[0]);
    });
  };
  const getJobById = () => {
    api
      .post('/project/getQuoteLineItemsById', { quote_id: JobId })
      .then((res) => {
        setJobLineItem(res.data.data);
        console.log('Job1', res.data.data);
        let grandTotal = 0;
        let grand = 0;
        let gst = 0;
  ...
  const GetPdf = () => {
    ...
    JoblineItem.forEach((element,index) => {
      lineItemBody.push([
        {
          text: `${index + 1}`,
          style: 'tableBody',
    ...
    const dd = {
      ...
      content: [
        ...
        {
          text: `Date :   ${(Job.quote_date)? moment(Job.quote_date).format('DD-MM-YYYY'):''}
           Job Code :  ${Job.quote_code ? Job.quote_code : ''
          }\n \n  `,
        ...
        },
        {
          text: `Dear Sir,
           ...
           With reference to the above captions, we would like to thank you for inviting us to Job for the above mentioned works and we are pleased to submit herewith our Value Quotation for you kind persual.`,
...
:- Payment : COD \n
:- The above Job does not cover replacement of any parts unless expressly stated above. \n
