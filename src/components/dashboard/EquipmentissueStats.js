...
const GoodsDeliveryChart = () => {
  ...
  const fetchDeliveryData = async (companyId) => {
    try {
      const { data } = await api.get('/equipmentrequest/getEquipmentStats', {
      ...
      const formattedData = data.data.map(item => ({
        ...
        y: item.equipment_request_count,  // Material request count on y-axis
        z: item.equipment_issue_count,  // Material issue count inside the bubble (point size)
      }));
  ...
  const optionsScatterChart = {
    ...
    markers: {
      ...
      strokeWidth: 0, // Adjust stroke width for better visibility
    },
    fill: {
      opacity: 0.8,
    },
    ...
    tooltip: {
      ...
      y: {
        formatter: (val) => `Equipment Request Count: ${val}`,
      },
      z: {
        formatter: (val) => `Equipment Issue Count: ${val}`,
      },
    },
    dataLabels: {
      enabled: true,
  ...
  return (
    <Row>
      <Col md="12">
        <ComponentCard title="Equipment Issue Statistics">
          ...
          {deliveryData && (
            <Chart options={optionsScatterChart} series={seriesScatterChart} type="scatter" height="400" width="1000" />
          )}
        </ComponentCard>
      </Col>
    </Row>
